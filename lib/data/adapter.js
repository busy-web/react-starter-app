...
function normalizeRecords(recordType, payload) {
	...
	return deserialize(recordType, data, { next, prev, rows: returned_rows, totalRows: total_rows, token: public_key });
}

function deserialize(recordType, data, meta) {
	let map = {};
	let model = data.map((d, idx) => {
		let json = {};
		Object.keys(d).forEach(k => json[camelize(k)] = d[k]);
		map[d.id] = idx;
		return json;
	});

	return { model, map, __meta: meta, __type: recordType };
}
